  ...
  return (
    <div className="w-full max-w-3xl px-6">
      <div className="relative group">
        ...
        <div className={`relative bg-panel-dark/80 backdrop-blur-md border border-white/10 rounded-2xl p-2 transition-all duration-300 ${isLoading ? 'border-primary/40' : 'group-hover:border-white/20'}`}>
          <div className="flex items-center gap-4 px-4 py-3">
            <span className={`material-symbols-outlined text-gray-400 text-2xl ${isLoading ? 'animate-spin text-primary' : ''}`}>
            ...
            />
            {value && !isLoading && (
               <button onClick={() => setValue("")} className="text-gray-500 hover:text-white transition-colors p-1">
